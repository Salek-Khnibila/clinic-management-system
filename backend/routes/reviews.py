...
def create_review():
    ...
    # NOTE: plus de mise à jour de users.note / users.avis — supprimées en migration
...
# Retourne tous les avis d'un médecin + moyenne et total calculés à la volée.
...
def get_doctor_reviews(medecin_id):
    ...
    rows = [serialize_row(r) for r in (reviews or [])]

    # Moyenne et total calculés à la volée — source unique de vérité
    total   = len(rows)
    avg     = round(sum(r["note"] for r in rows) / total, 1) if total > 0 else None
